...
const AgedPayableReport = () => {
    ...
    const supplierOptions = [
        ...
        // Add more supplier options here if available
    ];
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <select value={filters.supplier} onChange={(e) => handleFilterChange('supplier', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-4 p-4 bg-gray-50 rounded-lg border border-gray-100">
                    <div className="text-sm text-gray-700 font-medium">
                        {t('reports.suppliers.aged_payable_report.report_title') || 'Aged Payable Report By Method: Invoices From Date: 2026 February 1, Sunday To Date: 2026 February 28, Saturday For All Branches'}
                    </div>
                    ...
                    <table className="w-full min-w-[1000px]">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                                <th className="px-4 py-3 text-start text-sm font-medium text-gray-700">{t('reports.suppliers.aged_payable_report.supplier') || 'Supplier'}</th>
                                <th className="px-4 py-3 text-start text-sm font-medium text-gray-700">{t('reports.suppliers.aged_payable_report.journal_account') || 'Journal Account'}</th>
                                <th className="px-4 py-3 text-start text-sm font-medium text-gray-700">{t('reports.suppliers.aged_payable_report.today') || 'Today'}</th>
                                <th className="px-4 py-3 text-start text-sm font-medium text-gray-700">{t('reports.suppliers.aged_payable_report.day_1_30') || 'Day 1-30'}</th>
                                <th className="px-4 py-3 text-start text-sm font-medium text-gray-700">{t('reports.suppliers.aged_payable_report.day_31_60') || 'Day 31-60'}</th>
                                <th className="px-4 py-3 text-start text-sm font-medium text-gray-700">{t('reports.suppliers.aged_payable_report.day_61_90') || 'Day 61-90'}</th>
                                <th className="px-4 py-3 text-start text-sm font-medium text-gray-700">{t('reports.suppliers.aged_payable_report.day_91_plus') || 'Day 91+'}</th>
                                <th className="px-4 py-3 text-start text-sm font-medium text-gray-700">{t('reports.suppliers.aged_payable_report.total') || 'Total'}</th>
                            </tr>
                        </thead>

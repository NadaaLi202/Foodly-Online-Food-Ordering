...
const InventoryValueReport = () => {
    ...
    const fetchReport = async () => {
        ...
        try {
            ...
            if (filters.product) {
                processedData = processedData.filter(item => (item.productId === filters.product || item._id === filters.product));
            }

            // Apply client-side sorting
            if (filters.sortBy === 'highest_value') {
                processedData.sort((a, b) => (b.totalValue || 0) - (a.totalValue || 0));
            } else if (filters.sortBy === 'lowest_value') {
                processedData.sort((a, b) => (a.totalValue || 0) - (b.totalValue || 0));
            }
    ...
    const totalSaleValue = reportData.reduce((sum, item) => sum + ((item.quantity || 0) * (item.sellingPrice || 0)), 0);
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={() => {
                                if (reportData.length > 0) {
                                    exportInventoryValueToExcel(reportData.map(item => ({
                                        productName: item.name,
                                        code: item.code,
                                        quantity: item.quantity,
                                        unitPrice: filters.method === 'average_cost' ? item.averageCost : item.purchasePrice,
                            ...
                            <button onClick={() => {
                                if (reportData.length > 0) {
                                    const blob = buildInventoryValuePdf(reportData.map(item => ({
                                        productName: item.name,
                                        code: item.code,
                                        quantity: item.quantity,
                                        unitPrice: filters.method === 'average_cost' ? item.averageCost : item.purchasePrice,
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {loading ? (
                                    ...
                                    reportData.map((item, idx) => {
                                        const unitPrice = filters.method === 'average_cost' ? (item.averageCost || 0) : (item.purchasePrice || 0);
                                        const saleValue = (item.quantity || 0) * (item.sellingPrice || 0);
                                        const saleProfit = saleValue - (item.totalValue || 0);

                                        return (
                                            <tr key={item._id || idx} className="bg-white hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-3 text-sm text-gray-900">{unitPrice.toFixed(2)}</td>
                                                <td className="px-4 py-3 text-sm text-gray-900 font-medium">{(item.totalValue || 0).toFixed(2)}</td>
                                                <td className="px-4 py-3 text-sm text-gray-900">{(item.sellingPrice || 0).toFixed(2)}</td>
                                                <td className="px-4 py-3 text-sm text-gray-900 font-medium">{saleValue.toFixed(2)}</td>
                                                <td className="px-4 py-3 text-sm text-green-600 font-medium">{saleProfit.toFixed(2)}</td>
                                            </tr>
                                        );
                                    })
                                )}
                            ...
                            {reportData.length > 0 && (
                                <tfoot className="bg-gray-50 font-bold">
                                    <tr>
                                        ...
                                        <td className="px-4 py-3 text-sm text-green-600">{(totalSaleValue - totalValue).toFixed(2)}</td>
                                    </tr>
                                </tfoot>
                            )}
                        </table>

...
const Invoices = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.invoiceNumber.trim()) {
            newErrors.invoiceNumber = t('sales.invoices.invoice_number') + ' ' + t('sales.common.required');
        }
            ...
            <div className="p-6">
                {loading && !isModalOpen ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-gray-400">
                        ...
                        <p className="text-lg font-medium">{t('sales.invoices.no_invoices')}</p>
                        <p className="text-sm">{t('sales.invoices.start_creating')}</p>
                    </div>
                ) : (
                                ...
                                {invoices.map((invoice) => (
                                    <tr key={invoice._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-semibold">
                                            {invoice.total?.toFixed(2)} {t('sales.common.currency')}
                                        </td>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
                        ...
                        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
                            <h2 className={`text-xl font-bold text-gray-800 text-${i18n.language === 'ar' ? 'right' : 'left'}`}>{t('sales.invoices.add_invoice')}</h2>
                            <button
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder={t('sales.invoices.invoice_number_placeholder') || "INV-26-1-000001"}
                                            className={`w-full border-2 ${errors.invoiceNumber ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-indigo-500'} rounded-lg px-3 py-2.5 text-${i18n.language === 'ar' ? 'right' : 'left'} focus:outline-none transition-colors`}
                                        />
                                        {errors.invoiceNumber && (
                                            <p className={`text-red-500 text-xs mt-1 text-${i18n.language === 'ar' ? 'right' : 'left'} flex items-center gap-1`}>
                                                <span>⚠</span> {errors.invoiceNumber}
                                            </p>
                                        )}
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <input
                                            ...
                                            onChange={handleInputChange}
                                            placeholder={t('sales.common.client')}
                        ...
                        <div className="bg-white border-t border-gray-200 px-6 py-4 flex items-center justify-start gap-3 sticky bottom-0">
                            ...
                            >
                                {loading ? t('sales.common.saving') : t('sales.invoices.add_invoice')}
                            </button>
                        </div>
                    </div>
                </div>

...
const ProductLabels = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const { data } = await api.get('/templates?type=product-label');
    ...
    const handleDelete = async (id, e) => {
        ...
        try { await api.delete(`/templates/${id}`); setTemplates(t => t.filter(x => x._id !== id)); toast.success(t('Deleted successfully', 'تم الحذف')); }
        ...
        <div className="min-h-screen bg-[#f5f7f9]" dir={isRtl ? 'rtl' : 'ltr'}>
            <div className="flex items-center justify-between px-4 py-3">
                ...
                {/* <div className="flex items-center gap-1">
                    ...
                    <span className="px-3 h-9 flex items-center rounded bg-white border border-gray-200 text-sm font-semibold text-gray-800 shadow-sm">ملصقات المنتجات</span>
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    <table className="min-w-full">
                        <thead><tr className="border-b border-gray-200 bg-gray-50">
                            ...
                            <th className="px-4 py-3 text-start text-sm font-bold text-gray-700">{t('Size', 'الحجم')}</th>
                            <th className="px-4 py-3 w-10" />
                        </tr></thead>
                        <tbody>
                            {loading ? <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-400 text-sm">{t('Loading...', 'جارٍ التحميل...')}</td></tr>
                                : templates.length === 0 ? <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-400 text-sm">{t('No templates found', 'لا توجد قوالب')}</td></tr>
                                    : templates.map(tpl => (
                                        <tr key={tpl._id} onClick={() => navigate(`/dashboard/templates/product-labels/${tpl._id}/edit`)} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer">
                                            <td className="px-4 py-3 text-sm font-semibold text-gray-800">{tpl.name}</td>
                                            <td className="px-4 py-3 text-sm text-gray-500">{tpl.label?.width ?? 40} × {tpl.label?.height ?? 22} {t('mm', 'ملم')}</td>
                                            <td className="px-4 py-3 relative" onClick={e => e.stopPropagation()} ref={openMenu === tpl._id ? menuRef : null}>
                                                ...
                                                {openMenu === tpl._id && (
                                                    <div className={`absolute top-full mt-1 w-32 bg-white border border-gray-100 rounded-lg shadow-lg z-30 overflow-hidden ${isRtl ? 'left-0' : 'right-0'}`}>
                                                        <Link to={`/dashboard/templates/product-labels/${tpl._id}/edit`} className="flex items-center gap-2 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><Pencil size={14} className="text-gray-400" /> {t('Edit', 'تعديل')}</Link>
                                                        <button onClick={e => handleDelete(tpl._id, e)} className="flex items-center gap-2 w-full px-3 py-2.5 text-sm text-red-600 hover:bg-red-50"><Trash2 size={14} /> {t('Delete', 'حذف')}</button>
                                                    </div>
                                                )}
                                            </td>
                                        </tr>

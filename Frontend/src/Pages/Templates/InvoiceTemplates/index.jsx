...
const InvoiceTemplates = () => {
    ...
    const handleDelete = async (id, e) => {
        ...
        try { await api.delete(`/templates/${id}`); setTemplates(t_arr => t_arr.filter(x => x._id !== id)); toast.success(t('Deleted successfully', 'تم الحذف')); }
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    <table className="min-w-full">
                        <thead><tr className="border-b border-gray-200 bg-gray-50">
                            ...
                            <th className="px-4 py-3 text-start text-sm font-bold text-gray-700">{t('Template', 'القالب')}</th>
                                ...
                                : templates.length === 0 ? <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-400 text-sm">{t('No templates found', 'لا توجد قوالب')}</td></tr>
                                    : templates.map(tpl => (
                                        <tr key={tpl._id} onClick={() => navigate(`/dashboard/templates/invoices/${tpl._id}/edit`)} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer">
                                            ...
                                            <td className="px-4 py-3 text-sm text-gray-500">{tpl.designId ? `${t('Invoice Template', 'قالب الفاتورة')} ${tpl.designId.replace('design-', '')}` : `${t('Invoice Template', 'قالب الفاتورة')} 1`}</td>
                                            <td className="px-4 py-3 relative" onClick={e => e.stopPropagation()} ref={openMenu === tpl._id ? menuRef : null}>
                                                ...
                                                {openMenu === tpl._id && (
                                                    <div className={`absolute top-full mt-1 w-32 bg-white border border-gray-100 rounded-lg shadow-lg z-30 overflow-hidden ${isRtl ? 'left-0' : 'right-0'}`}>
                                                        <Link to={`/dashboard/templates/invoices/${tpl._id}/edit`} className="flex items-center gap-2 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><Pencil size={14} className="text-gray-400" /> {t('Edit', 'تعديل')}</Link>

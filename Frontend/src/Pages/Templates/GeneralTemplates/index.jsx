...
const GeneralTemplates = () => {
    ...
    const handleAdd = async () => {
        try {
            const { data } = await api.post('/templates', { name: 'قالب جديد', type: 'general' });
                            ...
                            {loading ? (
                                ...
                                <tr key={tpl._id} onClick={() => navigate(`/dashboard/templates/general/${tpl._id}/edit`)} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <td className="px-4 py-3 relative" onClick={e => e.stopPropagation()} ref={openMenu === tpl._id ? menuRef : null}>
                                        <button onClick={() => setOpenMenu(openMenu === tpl._id ? null : tpl._id)} className="p-1.5 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded transition-colors">

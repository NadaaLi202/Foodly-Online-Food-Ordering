...
const AccountingSettings = () => {
    ...
    const fetchSettings = async () => {
        try {
            const response = await api.get('/settings?category=accounting');
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            await api.patch('/settings/accounting', { settings });
                        ...
                        <div className="flex items-start">
                            <div className={`ml-3 mr-3 flex-1 ${i18n.language === 'ar' ? 'text-right' : 'text-left'}`}>
                                <p className="text-sm font-bold text-gray-900">
                                    {t('accounting_settings.success')}
                                </p>
                                <p className="mt-1 text-sm text-gray-500">
                                    {t('accounting_settings.save_success')}
                                </p>
                            </div>
            ...
            logError('Error saving accounting settings:', error);
            toast.error(t('accounting_settings.error_message'));
        } finally {
            setSaving(false);
        }
    };
    ...
    const tabs = [
        { id: 'general', label: t('accounting_settings.general_tab') }
    ];
        ...
        <div className={`min-h-screen bg-gray-50/50 pb-20 ${isRTL ? 'font-arabic' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
            ...
            <div className="bg-white border-b border-gray-200 mb-8">
                <div className="max-w-5xl mx-auto px-6 h-14 flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <ChevronRight className={`h-4 w-4 ${isRTL ? 'rotate-180' : ''}`} />
                    <span className="text-gray-900 font-medium">{t('accounting_settings.title')}</span>
                </div>
            </div>
            ...
            <div className="max-w-5xl mx-auto px-6">
                    ...
                    <div className="flex-1 p-10">
                        {activeTab === 'general' && (
                            <div className="space-y-1">
                                {[
                                    { key: 'enable_cost_centers', label: t('accounting_settings.enable_cost_centers') }
                                ].map((setting) => (
                                    <label
                                        key={setting.key}
                ...
                <div className={`mt-8 flex ${isRTL ? 'justify-start' : 'justify-end'}`}>
                    ...
                    >
                        ...
                        <span>{t('accounting_settings.save')}</span>
                    </button>
                </div>
            </div>
